 ...
 */
		...
		function drawtext(){
			...
			context.fillStyle='2A75A9';
			context.fillText("Are You Near?", 100, 50);
		...
		function drawDestinationcoords(){
			...
			context.fillText("45.5416, -122.9601", 90, 80); //Latitude, Longitude
		...
		//Check the window width for resizing the canvas, table headers & content
		function widthadapter(){
			//Original size of the canvas used
			//Screen width sizes to consider 320, 480, 540, 600, 768
			var canvas = document.getElementById("surface");
            var canvas_div = document.getElementById("div_canvas");
            var coord_tbl_hdr = document.getElementById("coord_plane_header");
            var lat_cell = document.getElementById("latitude");
            var longi_cell = document.getElementById("longitude");
 		...
 		window.addEventListener("load", widthadapter, true);
		...
		function updateLocation(position){
            ...
            time = String(time).substring(0,33);
